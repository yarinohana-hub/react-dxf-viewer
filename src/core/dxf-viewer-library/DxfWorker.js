...
export class DxfWorker {
    ...
    _CreateFontFetchers(urls, progressCbk) {
        ...
        function CreateFetcher(url, index) {
            return async function() {
                try {
                    ...
                    if (!response.ok) {
                        console.warn(`[DxfWorker] Font load failed (${response.status}): ${url}`)
                    ...
                    console.error(`[DxfWorker] Error loading font ${url}:`, error)
                    // Return null to allow fallback to next font
                    throw error
                }
            }
        }
        ...
        if (urls && urls.length > 0) {
            for (let i = 0; i < urls.length; i++) {
                fetchers.push(CreateFetcher(urls[i], i))
            }
        } else {
